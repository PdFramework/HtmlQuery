namespace PeinearyDevelopment.Framework.HtmlQuery
{
    ...
    internal static class HtmlParser
    {
        ...
        {
            var htmlPageStack = new Stack<DomElement>();
            ...
            while (!reader.EndOfStream())
            {
                ...
                if (element?.TagName != null && element.TagName.StartsWith("/"))
                {
                    var descendants = new List<DomElement>();
                    ...
                    //currentElement.Descendants = descendants;//TODO: research how to do this properly. this line was previously overwriting descendants with malformed html. now it keeps them, but the ordering of nodes seems to be incorrect
        ...
        internal static DomElement GetElement(this TextReader reader)
        {
            ...
            if (reader.Peek() != '<')
            {
                return new DomElement { Text = reader.GetText(), TagName = "TEXT" };
            }
            ...
            if (string.Equals(elementName, "script", StringComparison.OrdinalIgnoreCase) || string.Equals(elementName, "style", StringComparison.OrdinalIgnoreCase))
            {
                return reader.GetSpecialTag(elementName);
            }

            return new DomElement { Attributes = reader.GetElementAttributes(), TagName = elementName };
        }
        ...
        internal static DomElement GetComment(this TextReader reader)
        {
            reader.AdvanceReaderThroughWhiteSpace();
            ...
            return new DomElement { TagName = "COMMENT", Text = !string.IsNullOrWhiteSpace(strComment) ? strComment : null, Descendants = new List<DomElement>() };
        }

        internal static DomElement GetSpecialTag(this TextReader reader, string name)
        {
            reader.AdvanceReaderThroughWhiteSpace();

            var scriptElement = new DomElement
            {
        ...
        internal static IEnumerable<string> GetAttributeValue(this TextReader reader, string attributeName)
        {
            ...
            return valueString?.Length == 0 ? new string[] { } : new[] { valueString };
        }
